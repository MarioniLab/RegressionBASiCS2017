...
#### Create SummarizedExperiment objects for each condition

# Young active B6
input <- input[,grepl("SS51_active", colnames(input)) | grepl("SS52_active", colnames(input))]
chips <- sapply(colnames(input), function(n){unlist(strsplit(n, "_"))[1]})

Data.YoungActiveB6 <- newBASiCS_Data(Counts = input, Tech = grepl("ERCC", rownames(input)), SpikeInfo = SpikeInput.1, BatchInfo=chips)

#### Run MCMC on these conditions

# Young active B6

MCMC.YoungActiveB6 <- BASiCS_MCMC(Data.YoungActiveB6, 40000, 20, 20000, Regression = TRUE, k = 12, Var = 1.2, PrintProgress=FALSE)
