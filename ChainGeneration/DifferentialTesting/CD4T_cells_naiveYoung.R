...
# Young naive B6
input <- input[,grepl("SS51_naive", colnames(input)) | grepl("SS52_naive", colnames(input))]
chips <- sapply(colnames(input), function(n){unlist(strsplit(n, "_"))[1]})

Data.YoungNaiveB6 <- newBASiCS_Data(Counts = input, Tech = grepl("ERCC", rownames(input)), SpikeInfo = SpikeInput.1, BatchInfo = chips)

#### Run MCMC on these conditions

# Young naive B6

MCMC.YoungNaiveB6 <- BASiCS_MCMC(Data.YoungNaiveB6, 40000, 20, 20000, Regression = TRUE, k = 12, Var = 1.2, PrintProgress=FALSE)
