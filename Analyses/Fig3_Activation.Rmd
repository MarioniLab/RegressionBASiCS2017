---
title: "Fig3 Activation and ageing"
author: "Nils Eling"
date: "03/09/2017"
...
This analysis uses MCMC runs on CD4$^+$ T cells in following conditions: naive and activated in young B6, activated in young CAST, activated in old B6.

## Activation of CD4$^+$ T cells

Here we show the advantages of the BASiCS extension for differential variability testing.
...
A GO analysis will show which regulatory processes are present within the four groups.

```{r}
# We use the down- or up-regulated genes as background
write.table(Test.NaiveActive$TableMean$GeneName[Test.NaiveActive$TableMean$ResultDiffMean == "Group1+"],
            "Results/Differential_testing/CD4T_cells/Upregulated_genes.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)

write.table(Test.NaiveActive$TableMean$GeneName[Test.NaiveActive$TableMean$ResultDiffMean == "Group2+"],
            "Results/Differential_testing/CD4T_cells/Downregulated_genes.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)

write.table(Test.NaiveActive$TableResDisp$GeneName,
            "Results/Differential_testing/CD4T_cells/All_genes.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)

write.table(rownames(df)[df$Regulation == "Up-regulated, higher variability"],
            "Results/Differential_testing/CD4T_cells/CD4_Activation_UpMoreVar.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
cur_df <- df[df$Regulation == "Up-regulated, higher variability",]
cur_df$Names <- mouse.genes[rownames(cur_df),2]
write.csv(cur_df[order(cur_df$DistEpsilon, decreasing = TRUE),],
          "Results/Differential_testing/CD4T_cells/CD4_Activation_UpMoreVar.csv")

write.table(rownames(df)[df$Regulation == "Up-regulated, lower variability"],
            "Results/Differential_testing/CD4T_cells/CD4_Activation_UpLessVar.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
cur_df <- df[df$Regulation == "Up-regulated, lower variability",]
cur_df$Names <- mouse.genes[rownames(cur_df),2]
write.csv(cur_df[order(cur_df$DistEpsilon, decreasing = FALSE),],
          "Results/Differential_testing/CD4T_cells/CD4_Activation_UpLessVar.csv")

write.table(rownames(df)[df$Regulation == "Down-regulated, higher variability"],
            "Results/Differential_testing/CD4T_cells/CD4_Activation_DownMoreVar.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
cur_df <- df[df$Regulation == "Down-regulated, higher variability",]
cur_df$Names <- mouse.genes[rownames(cur_df),2]
write.csv(cur_df[order(cur_df$DistEpsilon, decreasing = TRUE),],
          "Results/Differential_testing/CD4T_cells/CD4_Activation_DownMoreVar.csv")

write.table(rownames(df)[df$Regulation == "Down-regulated, lower variability"],
            "Results/Differential_testing/CD4T_cells/CD4_Activation_DownLessVar.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
cur_df <- df[df$Regulation == "Down-regulated, lower variability",]
cur_df$Names <- mouse.genes[rownames(cur_df),2]
write.csv(cur_df[order(cur_df$DistEpsilon, decreasing = FALSE),],
          "Results/Differential_testing/CD4T_cells/CD4_Activation_DownLessVar.csv")
```

But we can look at single genes.
...
Now look at genes that increase in variability
