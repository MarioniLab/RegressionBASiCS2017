...
First look at the values for mu.
...
# Split genes into lowly, medium and highly expressing for new and old
...
Now look at the values for delta.

```{r}
# Split genes into highly and lowly expressing for new and old
...
Compute correlation between 10 replicates for the downsampling experiment and the true values for mu

```{r}
# Remove genes that are not used for fit
# And split genes into highly and lowly expressed
...
df.summary <- ddply(df.cors.mu, c("NoCells", "OldNew", "LowHigh"), summarise,
                    ...
                    Lower = quantile(Correlation, 0.025),
...
Compute correlation between 10 replicates for the downsampling experiment and the true values for delta.
...
Now look at the values for epsilon.
