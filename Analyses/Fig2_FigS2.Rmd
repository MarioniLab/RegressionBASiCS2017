...
of log2 fold changes (for $\delta_i$ and $\mu_i$) and absolute distances (for $\epsilon_i$)
...
for(i in seq(1,213, 3)){
  ...
  df.mu[counter,1] <- median(log2(cur_mu_old[log(mu_real) < lower] / 
                                    mu_real[log(mu_real) < lower]))
  
  df.mu[counter+1,1] <- median(log2(cur_mu_old[log(mu_real) > lower & log(mu_real) < higher] / 
                                      mu_real[log(mu_real) > lower & log(mu_real) < higher]))
  
  df.mu[counter+2,1] <- median(log2(cur_mu_old[log(mu_real) > higher] / 
                               mu_real[log(mu_real) > higher]))
  
  df.mu[counter+3,1] <- median(log2(cur_mu_new[log(mu_real) < lower] / 
                                    mu_real[log(mu_real) < lower]))
  
  df.mu[counter+4,1] <- median(log2(cur_mu_new[log(mu_real) > lower & log(mu_real) < higher] / 
                                      mu_real[log(mu_real) > lower & log(mu_real) < higher]))
  
  df.mu[counter+5,1] <- median(log2(cur_mu_new[log(mu_real) > higher] / 
                               mu_real[log(mu_real) > higher]))
  counter <- counter+6
  idx = idx+2
}
...
boxplot.mu <- ggplot(df.summary) + 
  ...
  geom_errorbar(aes(x = NoCells, ymin = Lower, ymax = Upper, colour=OldNew), width=0.25, position=position_dodge(width = 0.5)) + ylim(c(-0.3,0.3)) + 
  scale_color_manual(values = c("steelblue4", "coral3")) + ylab("log(mu)") + 
  xlab("Number of cells") + facet_grid(LowHigh~.)
```

Now we plot the values for $\delta_i$.
...
for(i in seq(2,213, 3)){
  ...
  df.delta[counter,1] <- median(log2(cur_delta_old[log(mu_real) < lower] / 
                                    delta_real[log(mu_real) < lower]))
  
  df.delta[counter+1,1] <- median(log2(cur_delta_old[log(mu_real) > lower & log(mu_real) < higher] / 
                                      delta_real[log(mu_real) > lower & log(mu_real) < higher]))
  
  df.delta[counter+2,1] <- median(log2(cur_delta_old[log(mu_real) > higher] / 
                               delta_real[log(mu_real) > higher]))
  
  df.delta[counter+3,1] <- median(log2(cur_delta_new[log(mu_real) < lower] / 
                                    delta_real[log(mu_real) < lower]))
  
  df.delta[counter+4,1] <- median(log2(cur_delta_new[log(mu_real) > lower & log(mu_real) < higher] / 
                                      delta_real[log(mu_real) > lower & log(mu_real) < higher]))
  
  df.delta[counter+5,1] <- median(log2(cur_delta_new[log(mu_real) > higher] / 
                               delta_real[log(mu_real) > higher]))
  counter <- counter+6
  idx = idx+2
}
...
```{r}
mu_real <- Data.old$MCMC_939_1.rds_mu
epsilon_real <- Data.new$MCMC_939_1.rds_epsilon
...
boxplot.epsilon <- ggplot(df.summary) + 
  ...
  geom_errorbar(aes(x = NoCells, ymin = Lower, ymax = Upper), colour="coral3", width=0.25, position=position_dodge(width = 0.5)) + ylab("log(mu)") + ylim(c(-0.15,0.15)) + 
  xlab("Number of cells") + facet_grid(LowHigh~.) 
